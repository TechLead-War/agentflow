...
def ensure_initial_commit(cwd: str = "."):
    ...
    if not has_commits(cwd):
        run_git(["commit", "--allow-empty", "-m", "Initial commit (agenthub)"], cwd=cwd)
...
def merge_branch(branch: str, cwd: str = ".", squash: bool = True) -> bool:
    ...
    try:
        if squash:
            ...
            # Commit the squash
            run_git(["commit", "-m", f"agenthub: merge {branch}"], cwd=cwd)
        else:
            run_git(["merge", "--no-ff", branch, "-m", f"agenthub: merge {branch}"], cwd=cwd)
        return True
    except GitError:
