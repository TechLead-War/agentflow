...
class _FallbackProgress:
    ...
    def __enter__(self):
        print(f"AgentHub \u2014 {self.state.run_id} \u2014 {self.state.phase}")
...
def show_status(repo_path: str = "."):
    """One-shot status display (for `agenthub status`)."""
    state = load_state(repo_path)
    if state is None:
        print("No active or recent AgentHub run found.")
        return
...
def _print_simple_status(state: RunState):
    ...
    print(f"\nAgentHub \u2014 {state.run_id} \u2014 {phase_label}")
