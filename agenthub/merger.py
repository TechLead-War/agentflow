...
async def _refresh_branch_before_merge(
    ...
    try:
        ...
        # Record pre-rebase SHA to detect no-op rebases
        pre_rebase_sha = git_ops.run_git(
            ["rev-parse", "HEAD"], cwd=str(worktree_dir),
        )
        ...
        # Skip re-review if the rebase didn't change anything (no-op rebase).
        # The diff is identical to what was already approved in the worker loop.
        post_rebase_sha = git_ops.run_git(
            ["rev-parse", "HEAD"], cwd=str(worktree_dir),
        )
        if pre_rebase_sha == post_rebase_sha:
            log_round(
                repo_path, state.run_id, task.id, merge_review_round, "merge-review",
                "Rebase was a no-op (base branch unchanged). "
                "Skipping re-review — diff identical to the approved version.",
            )
