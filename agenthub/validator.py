...
async def validate(
    ...
    # Optionally run build/tests (async to avoid blocking the event loop)
    build_output = await _detect_and_run_tests(repo_path)
...
async def _detect_and_run_tests(repo_path: str) -> str:
    ...
    try:
        ...
        stdout, stderr = await asyncio.wait_for(
            proc.communicate(), timeout=_TEST_RUNNER_TIMEOUT_SEC,
        )
        output = (
